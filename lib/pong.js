...
var pong = {
  ...
  getPlayerSearchQuery: function (player_name, not_equal) {
    var q = null;
    if (player_name[0] === '<' && player_name[1] === '@' && player_name[player_name.length - 1] === '>') {
      var name = player_name.substr(2, player_name.length - 3)
      if (not_equal) {
        q = { user_id: { $ne: name } };
      } else {
        q = { user_id: name };
  ...
  findPlayer: function (player_name) {
    ...
    // If this is a player object then just return it.
    if (typeof player_name === 'object' && player_name._id) {
      return player_name;
    }

    var q = pong.getPlayerSearchQuery(player_name);
  ...
  findRandomPlayer: function(except_player_name) {
    var exception_query = pong.getPlayerSearchQuery(except_player_name, true);

    return Player.count(exception_query).then(function(count) {
      if (count < 2) {
        return new Error("Not enough players registerd")
      }

      var rand = _.random(0, count - 1);
      return Player.where(exception_query).findOne().skip(rand).exec();
    });
  },
  ...
  createSingleRandomChallenge: function(c1) {
    return pong.findRandomPlayer(c1).then(function (c2) {
      return pong.createSingleChallenge(c1, c2);
    });
  },
