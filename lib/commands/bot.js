...
var LogCommand = {
  command: /^log(\s+(\d+))?$/i,
  run: function logCommandRun(params, hook, res) {
    var logN = null;
    if (params[2] && !isNaN(parseFloat(params[2])) && isFinite(params[2])) {
      logN = +params[2];
    } else {
      logN = 5;
    }
    ...
    if (topN <= 0) {
      return res.json({ text: "Invalid params, use _pongbot leaderboard <1-Infinity>_." });
    }

    Challenge.find({state: 'Finished'}, {limit: topN}).then(function (challenges) {
